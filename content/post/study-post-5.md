---
title: "資料庫應用 Part II" #輸入這篇的title
date: "2024-08-31" #輸入時間
draft: true #若為false就會直接刊出，不會以草稿模式運作
toc: false #自動產生TOC
thumbnail: "/images/study-post-5.png" #每篇文章的縮圖位置在這裡(static/images/)-變更
codeMaxLines: 10 # Override global value for how many lines within a code block before auto-collapsing.
codeLineNumbers: false # Override global value for showing of line numbers within code block.
figurePositionShow: true # Override global value for showing the figure label.
categories:
  - study
tags:
  - Roam research
  - NotebookLM
  - Whisper
  - ChatGPT
  - Perplexity
  - AI工具
---

![第二大腦(Roam Research)](/images/ipic/c8y6kb.png)

年初講到[資料庫的應用與靈感筆記如何建立?](https://agoodbear.github.io/post/study-post-2/)，有興趣可以點選回到這邊看看。

內容主要為**靈感筆記**的建立，與整體資料庫的應用。

---

將自己認可的資訊來源抓入資料庫，這個動作就是primary selection。

這個動作有點像儲存全文。

做這件事的前提是，對這篇文章有興趣。然後儲存到primary selection的資料庫。

目前我primary selection的資料庫為Readwise。

![Extension](/images/ipic/2kzi7v.png)

可以在自己的Browser上安裝這個Extension。作為全文的抓取，也可以除去在頁面中出現的一堆廣告之類，讓整個抓取的頁面乾淨一點。
