---
title: "心臟警訊：Pericarditis vs. OMI— 再次入院" #輸入這篇的title
date: "2024-02-02" #輸入時間
draft: true #若為false就會直接刊出，不會以草稿模式運作
featured: false #若設定為true，會設定為精選文章
toc: false #自動產生TOC
thumbnail: "/images/ecg-post-3.png" #每篇文章的縮圖位置在這裡(static/images/)-變更
...
![第一次急診就醫的ECG](https://miro.medium.com/v2/resize:fit:1400/1*tprAYzfatLfq__U6OEglhw.png)
...
![img](https://miro.medium.com/v2/resize:fit:1400/1*7uZfr7ftD9OVd3BKPzzkaQ.png)
...
![](https://miro.medium.com/v2/resize:fit:1400/1*yll2nGUvdkL-QG8ulM664Q.jpeg)
...
**Axis判讀我推薦可以玩看看[這個網站](https://david-shrk.github.io/ecgaxistrainer/)，把指針轉轉，看看I/aVF/II的變化來了解axis目前方向為何~~~**

Interval: No QT prolong

Ischemia:首要目標是找看看**有無OMI ECG findings(目前是18個…以後只會越來越多XD)**

![根據QRSTU判斷](https://miro.medium.com/v2/resize:fit:6904/1*G2QwkE0DPnULRuC0psCX3A.png)

![根據STD判斷](https://miro.medium.com/v2/resize:fit:6904/1*2gSPaVcRJw9P3SqzOPJJCw.png)

![特別Pattern](https://miro.medium.com/v2/resize:fit:6908/1*qkWpIWHGP_lVUNMz-42Iig.png)

![特別計算](https://miro.medium.com/v2/resize:fit:6900/1*LhHja5QM_5Nw144hVZRwWg.png)

![其他](https://miro.medium.com/v2/resize:fit:6900/1*2NSeEeAzOLBFJz_k3HAR9A.png)
...
![](https://miro.medium.com/v2/resize:fit:1400/1*3hhNNLZzMH2-y8AUe76G_g.png)
...
![擋下OMI病患](https://miro.medium.com/v2/resize:fit:1400/1*yIpVpxmAMcDVD24H1Zpn_A.png)

## 我們到底要擋下怎樣的OMI病患?

**STEMI(+)/OMI(+)**與**STEMI(-)/OMI(+)**
...
### <mark>擋下的意思，有分幾個層次(**請見Fig.8**)<mark>

Fig.8的『1』評鑑都在評了(**Guideline也是寫<10分鐘啦**)，有人會不敢10分鐘內做到ECG嗎?(**Door to ECG time**)

『2』做完後到臨床醫師判讀，這通常取決於臨床醫師，因為做完後，應該很快就會放到醫師桌面上。除非醫師沒看到或不曉得有胸痛病患的Chart在桌面上。(**2的時間幾乎可以忽略，若有增加，可以檢討為何做完ECG那麼久還沒被臨床醫師判讀**)

『3』我覺得非常非常的重要。這段時間代表臨床醫師開始判讀ECG，到臨床醫師認為要啟動心導管。這段時間叫做**ECG to Activation time(ETA)**。(**Ps:注意是同一個人，並不是CV man認為要啟動心導管，而是判讀的醫師認為要啟動心導管**)
...
ACC/AHA的D2B time包含了上述的時間，希望D2B time <90分鐘 。[^1]
...
- 辨識不出OMI →但是懂得**運用以下輔助工具與病患feature** →若(+)，**Call CV man**

1. Serial F/U ECG+Enzyme(在initial ECG後，覺得ECG有問題，但是又不符合STEMI criteria，然後OMI ECG findings的18個sign，又不是很熟，但就是認為ECG有問題，請千萬記得**10–15分鐘就要F/U ECG**)
2. 可以利用POCUS看看有無RWMA
3. 如果病患不符合STEMI criteria，但是出現NSTE-ACS的Very high risk feature的狀況 →**Call CV man(之前Blog多次提到，不再重複了啦)**
...
這邊推薦閱讀一篇McLaren在2021年發表在AJEM的文章。[^2]

文章中各以一年為期分成pre-intervention和post-intervention來做比較:

![img](https://miro.medium.com/v2/resize:fit:1400/1*pgRxwBSmfSC3qPIT780luQ.png)

![img](https://miro.medium.com/v2/resize:fit:1400/1*d49jJzd0TtZbQRtHVbmGfw.png)
...
![img](https://miro.medium.com/v2/resize:fit:1400/1*afZ50XjC1uQd9bR6j7eGag.png)
...
在Uptodate的這篇文章中 [^3] 說明在一項研究中118位被診斷acute idiopathic pericarditis，發現有38位病患(32%)，有biomarker上升。但是這些病患也應該要考慮是否有myopericarditis。(**Pericarditis不到一半會biomarker上升)**

Dr.Smith有一篇文章關於診斷pericarditis，非常非常的經典。[^4]
...
![img](https://miro.medium.com/v2/resize:fit:1400/0*4k0fIDolonxtHjkr)
...
在這篇文章裡面  [^6] ，我們學到了關於Exercise stress test的幾個重點:

⭐️**Negative exercise stress test對我們ED man沒有任何用處**
...
因為我們可能**想像Negative exercise stress test = 血管沒有問題(但事實並非如此)**
...
是不是會導致ACS →**Stress test and Cath不是幫助那麼大** →這可以從**ACS病生理機轉**來解釋清楚

**plaque vulnerability(容易受傷)和穩定度**對於造成ACS的重要影響程度 >> plaque size與管徑狹窄程度
...
![img](https://miro.medium.com/v2/resize:fit:1400/0*JQiiaJCryLJQWQv2)

**Fig.5**

Fig.5 說明了並**不是所有的plaques都是一樣的**
...
![img](https://miro.medium.com/v2/resize:fit:1400/0*PNleVhu3rWOTRaXh)
...
![img](https://miro.medium.com/v2/resize:fit:1400/0*QAe8hsCq6ofBdJA3)
...
綜合上述可以知道即使**近期stress test or CAG是Negative，也不能完全排除ACS(沒有任何一種處置可以讓ACS的風險降到零) →解決方法:病史很重要、ECG、TnI、validated ADP(HEART score等)**
...
![img](https://miro.medium.com/v2/resize:fit:1400/1*jwIsjSB0QRkbTfjtHSwo5Q.png)

**Fig.8 再次返診的initial ECG**

Fig.8的ECG看到了哪些ischemic ECG morphology?

1. 首先V1-V4有LV aneurysm的morphology，所以可以應用Dr.Smith的T/QRS >0.36的應用 →在V2/V3肯定超過0.36
2. Straight ST segment over I/aVL、Maybe STD in III
3. V4-V6的STE是否有意義?我覺得有看到J wave出現，所以我覺得比較像early repolarization造成的STE
...
![img](https://miro.medium.com/v2/resize:fit:1400/1*WjoHV4N8M-wE19EvVvy2rg.png)
...
![img](https://miro.medium.com/v2/resize:fit:1400/1*yIpVpxmAMcDVD24H1Zpn_A.png)

**Fig.3**

## **CAG final result: Acute D1 total occlusion**

![img](https://miro.medium.com/v2/resize:fit:1400/0*Lc2ETQOv7bvgj2Sr)
...
1. 18個OMI ECG findings有哪些?
2. 如何擋下OMI病患
3. 降低ETA的方法
4. 我們真的要診斷pericarditis嗎?
...
[^4]:Dr. Smith’s ECG Blog: You Diagnose Pericarditis at your Peril (at the Patient’s Peril!) — [link](http://hqmeded-ecg.blogspot.com/2018/06/you-diagnose-pericarditis-at-your-peril.html)
[^5]:Dr. Smith’s ECG Blog: ST elevation after gunshot to the chest — [link](http://hqmeded-ecg.blogspot.com/2022/06/st-elevation-after-gunshot-to-chest.html)
[^6]:Chest Pain, “Negative” Stress Tests, POCUS, & ECG Equations — REBEL EM — Emergency Medicine Blog — [link](https://rebelem.com/chest-pain-negative-stress-tests-pocus-ecg-equations/)
...
7. Amal Mattu’s ECG Case of the Week — March 8, 2021 — ECG Weekly — [link](https://ecgweekly.com/2021/03/amal-mattus-ecg-case-of-the-week-march-8-2021/)